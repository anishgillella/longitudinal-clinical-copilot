...
1. OBSERVED (High confidence) - Patterns directly observable in how the patient speaks:
   - Echolalia: Patient repeats the AI's words back verbatim
...
2. SELF_REPORTED (Medium confidence) - Patient describes their experiences:
   - "The lights hurt my eyes" → sensory sensitivity
   - "I don't like when plans change" → routine rigidity
   - "I play alone at recess" → social preference
   - "I don't know how I feel" → emotional identification difficulty
   - "Kids at school are confusing" → social understanding challenges

3. INFERRED (Lower confidence) - Patterns interpreted from context:
   - Consistently short responses to social questions → possible social difficulty
   - Detailed responses only about specific topics → possible restricted interests
   - Difficulty with open-ended emotion questions → emotional processing
   - Avoidance of certain question topics → possible sensory/social aversion

Signal categories:
- COMMUNICATION: Speech patterns, pragmatic language, conversation flow
- SOCIAL: Relationship descriptions, social preferences, peer interactions
- SENSORY: Sensitivity descriptions, seeking behaviors, aversions
- BEHAVIORAL: Routines, repetitive actions, intense interests
- EMOTIONAL: Emotion identification, regulation, expression

IMPORTANT: For each signal, you MUST:
- Copy the EXACT QUOTE verbatim from the transcript as evidence
- Specify evidence_type: observed, self_reported, or inferred
- Include reasoning explaining WHY this is clinically significant
- Rate confidence based on evidence type (observed > self_reported > inferred)"""
...
1. signal_type: communication, social, sensory, behavioral, or emotional
2. signal_name: brief descriptive name (e.g., "Echolalia", "Sensory sensitivity to light")
3. evidence: EXACT QUOTE copied verbatim from transcript
4. evidence_type: observed, self_reported, or inferred
5. reasoning: WHY this is clinically significant (1-2 sentences)
6. transcript_line: approximate line number where this appears in transcript
7. intensity: 0.0-1.0 (how prominent is this signal)
8. confidence: 0.0-1.0 (observed=0.8+, self_reported=0.5-0.8, inferred=0.3-0.5)
9. maps_to_domain: which assessment domain code this relates to
10. clinical_significance: low, moderate, or high
...
{{
    "signals": [
        {{
            ...
            "reasoning": "why this is clinically significant",
            "transcript_line": 0,
            "intensity": 0.0,
            "confidence": 0.0,
            "maps_to_domain": "...",
    ...
    "limitations": {{
        "not_assessable": ["list areas that cannot be assessed from transcript alone, e.g., eye contact, motor behaviors"],
...
6. Weight evidence by type: OBSERVED > SELF_REPORTED > INFERRED
7. Link each piece of evidence to its source signal ID
...
IMPORTANT: When citing evidence, ALWAYS reference the signal_id so the clinician can click through to see the original transcript context.
...
IMPORTANT: Each evidence point must reference the signal_id from the signals list above.

Return as JSON:
{{
    "hypotheses": [
        {{
            "condition_code": "asd_level_1|asd_level_2|asd_level_3|sensory_processing|adhd|anxiety|no_asd|insufficient_data",
            ...
            "contradicting_evidence": [
                {{
                    ...
                    "reasoning": "why this is contradicting"
                }}
            ],
            "explanation": "Clear, detailed reasoning for this hypothesis (2-3 sentences)",
            "limitations": "What cannot be assessed from transcript alone"
        }}
    ],
    "differential_considerations": [
        {{
            "condition": "Other condition name",
            "reasoning": "Why this should be considered",
            "distinguishing_features": "What would help differentiate"
        }}
    ],
    "evidence_gaps": [
        {{
            "area": "Area needing more information",
    ...
    "confidence_statement": "Overall statement about confidence in these hypotheses, acknowledging limitations of transcript-only analysis"
}}"""
